...
public class Main {
	...
	public static void print_help() {
		System.out.println("Usage: java [-cp javmaop_classpath] llvmmop.TestSuite [-options] <test suite dir>");
		System.out.println("");
		...
		System.out.println("    -m <llvmmop dir path>\t  select the llvmmop directory");
		System.out.println();
	}
	...
	static public boolean process(String testSuiteDirPath) {
		...
		try {
			...
			title3 = "*** into AspectJ monitors (using llvmmop) ***";
			System.out.println(repeatChar('*', title1.length()));
			System.out.println(title1);
			System.out.println(title2);
			System.out.println(title3);
			System.out.println(repeatChar('*', title1.length()));
			allPassed = traverseTestCaseDir(rootDir, new LLVMMOPCompileFunctor());
			System.out.println();
	...
	public static void main(String[] args) {
		...
		if (!Main.isJarFile && !new File(Main.javamopDir).exists()) {
			System.err.println("[ERROR] the given llvmmop directory does not exist.");
			return;
		}
		try {
