...
public class LLVMMOPCompileFunctor implements TestCaseFunctor {
		...
		for (TestCaseSpec testCaseSpec : testCase.specFiles) {
			...
			if (Main.isJarFile) {
				if (Main.local) {
					...
					String[] cmdarray2 = { "java", "-cp", Main.jarFilePath + File.pathSeparator + logicRepositoryPath, "llvmmop.LLVMMOPMain",
							server, "-v", "-debug", testCase.basepath + File.separator + testCase.path + File.separator + testCaseSpec.spec_filename };
					cmdarray = cmdarray2;
				} else {
					String[] cmdarray2 = { "java", "-cp", Main.jarFilePath, "llvmmop.LLVMMOPMain", server, "-v", "-debug",
							testCase.basepath + File.separator + testCase.path + File.separator + testCaseSpec.spec_filename };
					cmdarray = cmdarray2;
				}
			} else {
				String[] cmdarray2 = { "java", "-cp", Main.javamopDir, "llvmmop.LLVMMOPMain", server, "-v", "-debug",
						testCase.basepath + File.separator + testCase.path + File.separator + testCaseSpec.spec_filename };
				cmdarray = cmdarray2;
			}
