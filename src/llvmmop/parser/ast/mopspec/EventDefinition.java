...
import llvmmop.parser.ast.stmt.*;
import llvmmop.parser.ast.visitor.*;
import llvmmop.parser.ast.type.*;
...
public class EventDefinition extends Node {
	...
	private PointCut parsePointCut(String input) throws llvmmop.parser.main_parser.ParseException {
		...
		try {
			...
			throw new llvmmop.parser.main_parser.ParseException("The following error encountered when parsing the pointcut in the event definition: "
					+ e.getMessage());
		}
		...
		if (threadVar == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one thread() pointcut.");
		...
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("thread() pointcut should appear at the root level in a conjuction form");
		
		// thread name pointcut
		threadNameVar = resultPointCut.accept(new ThreadNameVarVisitor(), null);
		if (threadNameVar == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one threadName() pointcut.");
		if (threadNameVar.length() != 0) {
			resultPointCut = resultPointCut.accept(new RemoveThreadNameVisitor(), new Integer(1));
		} 
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("threadName() pointcut should appear at the root level in a conjuction form");
		...
		if (blockedThreads == null) {
			throw new llvmmop.parser.main_parser.ParseException("threadBlocked() should have one parameter.");
		} 
		...
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("threadBlocked() pointcut should appear at the root level in a conjuction form");

		
		// condition pointcut
		condition = resultPointCut.accept(new ConditionVisitor(), null);
		if (condition == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one condition() pointcut.");
		...
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("condition() pointcut should appear at the root level in a conjuction form");

		
		// Count condition pointcut
		countCond = resultPointCut.accept(new CountCondVisitor(), null);
		if (countCond == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one countCond() pointcut.");
		if (countCond.length() != 0) {
			resultPointCut = resultPointCut.accept(new RemoveCountCondVisitor(), new Integer(1));
		}
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("countCond() pointcut should appear at the root level in a conjuction form");
		
		// endProgram pointcut
		String checkEndProgram = resultPointCut.accept(new EndProgramVisitor(), null);
		if (checkEndProgram == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one endProgram() pointcut.");
		...
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("endProgram() pointcut should appear at the root level in a conjuction form");

		// endThread pointcut
		String checkEndThread = resultPointCut.accept(new EndThreadVisitor(), null);
		if (checkEndThread == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one endThread() pointcut.");
		...
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("endThread() pointcut should appear at the root level in a conjuction form");
		if (endProgram && endThread)
			throw new llvmmop.parser.main_parser.ParseException("endProgram() pointcut and endThread() pointcut cannot appear at the same time");

		// startThread pointcut
		String checkStartThread = resultPointCut.accept(new StartThreadVisitor(), null);
		if (checkStartThread == null)
			throw new llvmmop.parser.main_parser.ParseException("There are more than one startThread() pointcut.");
		...
		if (resultPointCut == null)
			throw new llvmmop.parser.main_parser.ParseException("startThread() pointcut should appear at the root level in a conjuction form");
		if (endThread && startThread)
			throw new llvmmop.parser.main_parser.ParseException("startThread() pointcut and endThread() pointcut cannot appear at the same time");
		...
		if (endObjectId == null || (endObjectId.length() != 0 && endObjectType == null))
			throw new llvmmop.parser.main_parser.ParseException("There are more than one endObject() pointcut.");
		...
		if (endObject && (endProgram || endThread))
			throw new llvmmop.parser.main_parser.ParseException("endProgram() pointcut, endThread(), and endObject() pointcut cannot appear at the same time");
	...
	public boolean has__STATICSIG() {
		...
		if (this.getAction() != null) {
			String eventAction = this.getAction().toString();
			// Nice! (This was copied from the LLVMMOPSpec class.)
