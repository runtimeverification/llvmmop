...
import llvmmop.parser.astex.visitor.CollectEventPointCutVisitor;
import llvmmop.parser.astex.visitor.CollectHandlerPointCutVisitor;
import llvmmop.parser.astex.visitor.RenameVariableVisitor;
import llvmmop.parser.astex.visitor.ReplacePointCutVisitor;
import llvmmop.parser.main_parser.LLVMMOPParser;
import llvmmop.util.Pair;
...
class SpecContext {
	...
	public SpecContext(LLVMMOPSpecExt spec, MOPSpecFileExt currentFile, HashMap<String, MOPSpecFileExt> depFiles) {
		this.spec = spec;
...
public class LLVMMOPExtender {
	public static MOPSpecFile translateMopSpecFile(MOPSpecFileExt currentFile) throws MOPException {
		...
		for (LLVMMOPSpecExt spec : currentFile.getSpecs()) {
		...
		for (LLVMMOPSpecExt spec : currentFile.getSpecs()) {
			LLVMMOPSpec spec2 = translateJavaMopSpec(spec, currentFile, depFiles);
			specList.add(spec2);
		}
	...
	protected static LLVMMOPSpec translateJavaMopSpec(LLVMMOPSpecExt spec, MOPSpecFileExt currentFile, HashMap<String, MOPSpecFileExt> depFiles)
			throws MOPException {
		...
		LLVMMOPSpec ret;
		try {
			ret = new LLVMMOPSpec(currentFile.getPakage(), spec.getBeginLine(), spec.getBeginColumn(), spec.getModifiers(), spec.getName(), spec.getParameters().toList(),
	...
	private static List<EventDefinition> collectAndTranslateEvents(SpecContext context) throws MOPException {
		...
		if (context.spec.hasExtend()) {
			for (ExtendedSpec parentSpecName : context.spec.getExtendedSpec()) {
				Pair<LLVMMOPSpecExt, MOPSpecFileExt> parentSpecPair = findJavaMOPSpec(parentSpecName.getName(), context.currentFile, context.depFiles);

				LLVMMOPSpecExt parentSpec = parentSpecPair.left;
	...
	private static PointCut resolveHandlerPointCuts(PointCut pointCut, EventDefinitionExt event, SpecContext context) throws MOPException {
		...
		for (HandlerPointCut handlerPointCut : handlerPointCuts) {
			...
			LLVMMOPSpecExt spec = targetPropExtPair.right.spec;
			...
			try {
				targetPointCut = AspectParser.parse(new ByteArrayInputStream(methodPatternStr.getBytes()));
			} catch (Exception e) {
	...
	private static int numProperties(LLVMMOPSpecExt spec) {
		int numProps = 0;
	...
	private static HashMap<String, MOPSpecFileExt> retrieveParentFiles(LLVMMOPSpecExt spec, MOPSpecFileExt currentFile) throws MOPException {
		...
		for (ExtendedSpec extSpec : spec.getExtendedSpec()) {
			...
			try {
				parentSpecFile = LLVMMOPParser.parse(parentFile);
			} catch (Exception e) {
			...
			for (LLVMMOPSpecExt spec2 : parentSpecFile.getSpecs()) {
	...
	private static LLVMMOPSpecExt findJavaMOPSpec(String name, MOPSpecFileExt currentFile) {
		for (LLVMMOPSpecExt spec : currentFile.getSpecs()) {
	...
	private static Pair<LLVMMOPSpecExt, MOPSpecFileExt> findJavaMOPSpec(String name, MOPSpecFileExt currentFile,
			HashMap<String, MOPSpecFileExt> depFiles) throws MOPException {
		LLVMMOPSpecExt parentSpec = findJavaMOPSpec(name, currentFile);

		if (parentSpec != null)
			return new Pair<LLVMMOPSpecExt, MOPSpecFileExt>(parentSpec, currentFile);
		...
		if (specFile != null) {
			...
			if (parentSpec != null)
				return new Pair<LLVMMOPSpecExt, MOPSpecFileExt>(parentSpec, specFile);
		}

		throw new MOPException("cannot find a parent specification: " + name);
	}

	private static List<BodyDeclaration> collectDeclarations(LLVMMOPSpecExt spec, MOPSpecFileExt currentFile, HashMap<String, MOPSpecFileExt> depFiles)
			throws MOPException {
		...
		for (ExtendedSpec parentSpecName : spec.getExtendedSpec()) {
			Pair<LLVMMOPSpecExt, MOPSpecFileExt> parentSpecPair = findJavaMOPSpec(parentSpecName.getName(), currentFile, depFiles);
	...
	private static List<EventDefinitionExt> collectNonImpAbstractEvents(LLVMMOPSpecExt spec, MOPSpecFileExt currentFile,
			HashMap<String, MOPSpecFileExt> depFiles) throws MOPException {
